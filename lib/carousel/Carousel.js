...
(function () {
    function definition($) {
        ...
        proto.init = function ($node, options) {
            ...
            // for (var i = 0, il = options.layers.length; i < il; i++) {
            //     options.layers[i].serve(this);
            // }
        ...
        proto.pauseAutoRun = function () {
            if (this._autoRunIntervalId) {
                //console.log('clear run interval id: ', this._autoRunIntervalId)
                clearInterval(this._autoRunIntervalId);
            }
        };
        ...
        proto.park = function () {
            //console.log('==========park: ', this._currentIndex)
        ...
        proto.setCurrentIndex = function (index, animate, direction, afterDrag, force) {
            ...
            $(this).trigger(event, { oldIndex: this._currentIndex, newIndex: index });
            return true;
        };
        proto.layout = function (wind, docd, resing) {
            this._haulier.layout(wind, docd, resing);
            this.refreshCache();
            // for (var i = 0, il = this._layers.length; i < il; i++) {
            //     this._layers[i].layout(wind, docd, resing);
            // }
        };
    ...
    //     define(['hance', 'jquery', 'crown/ui/carousel/AdaptiveHaulier', 'crown/ui/carousel/PrevNextLayer',
    // 'crown/ui/carousel/StampsLayer', 'modernizr'], definition);
    // } else if (typeof module !== 'undefined' && module.exports) {
    //     module.exports = definition(require('hance'), require('jquery'), require('./AdaptiveHaulier'), require('./PrevNextLayer'), require('./StampsLayer'));
    // } else {
    definition($);
    // }
})();